...
/**
 * generates changelog for a given version
 * @param  {String} version     version of the release
 * @param  {String} [from]      tag from which to start the changelog recap
 * @param  {String} [file]      file to store the changelog
 * @returns {String}            changelog of the commits
 */
var generate = function(version, from, file) {
    return getPreviousTag()
        ...
        .then((tag) => {
            return readGitLog('^fix|^feat|^perf|BREAKING', tag);
        })
        .then(function(commits) {
            //console.log('Parsed', commits.length, 'commits');
            //console.log('Generating changelog to', file || 'stdout', '(', version, ')');
            var stream = _.isString(file) ? fs.createWriteStream(file) : new MemoryStream();
